...
const DEFAULT_URL = './snapliiCreditAccountAgreement.pdf'
...
const PDFViewerApplication = {
    ...
    return loadingTask.promise.then(
      ...
      function (exception) {
        ...
        if (exception instanceof pdfjsLib.InvalidPDFException) {
          ...
          loadingErrorMessage = l10n.get('pdfjs-missing-file-error', null, 'Missing PDF file.')
  ...
  get loadingBar() {
    ...
    return pdfjsLib.shadow(this, 'loadingBar', new pdfjsViewer.ProgressBar(bar))
  },
  ...
  error: function pdfViewError(message, moreInfo) {
    ...
    if (moreInfo) {
      moreInfoText.push(
        l10n.get('pdfjs-error-message', { message: moreInfo.message }, 'Message: { $message }')
      )
      if (moreInfo.stack) {
        moreInfoText.push(
          l10n.get('pdfjs-error-stack', { stack: moreInfo.stack }, 'Stack: { $stack }')
        )
      } else {
        if (moreInfo.filename) {
          moreInfoText.push(
            l10n.get('pdfjs-error-file', { file: moreInfo.filename }, 'File: { $file }')
          )
        }
        if (moreInfo.lineNumber) {
          moreInfoText.push(
            l10n.get('pdfjs-error-line', { line: moreInfo.lineNumber }, 'Line: { $line }')
          )
        }
      }
    }
